...
if not tweets_df.empty:
    ...
    # Gérer correctement le cas où tweet_date_range est un tuple ou une seule date
    if isinstance(tweet_date_range, tuple):
        if len(tweet_date_range) >= 2:
            start_date, end_date = tweet_date_range[0], tweet_date_range[1]
        else:
            start_date = end_date = tweet_date_range[0] if tweet_date_range else min_date
    else:
    ...
    if tweets_filtered.empty:
        st.warning(f"Aucun tweet trouvé pour la période du {start_date} au {end_date}")
        # Afficher un message d'information
        st.info("Veuillez sélectionner une autre plage de dates")
    else:
